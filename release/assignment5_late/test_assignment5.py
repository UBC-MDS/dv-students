...
def test_1_3_2(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert sha1(answer.encoding.x['shorthand'].encode('utf8')).hexdigest() == "09ade1377f03dadec1de00aaa6fd4ac215edac2c", "Your x-axis is incorrect. Are you examining the original plot correctly?"
    assert sha1(answer.encoding.y['shorthand'].encode('utf8')).hexdigest() == "02cd3b107a9c91476522398de1fd67d78fffb72d", "Your y-axis is incorrect. Are you examining the original plot correctly?"
    return(success)
...
def test_1_5(answer):
    ...
    assert answer.encoding.color['shorthand'] == 'count()', "Make sure you are coloring by count."
    return(success)
...
def test_2_3(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert sha1(answer.encoding.color['shorthand'].encode('utf8')).hexdigest() == "30603fa9e0f620c305cd627ab0ff138a960c48bd", "Your color is incorrect. Are you setting the correct column to the color channel?"
    assert sha1(answer.to_dict()['encoding']['x']['aggregate'].encode('utf8')).hexdigest() == "09f3246f599ac9a17e658bc0657edea466004792", "Your aggregate function is incorrect. The median function might be useful here?"
    
    assert sha1(answer.to_dict()['encoding']['x']['field'].encode('utf8')).hexdigest() == "2bf5eebcf9e1dc77882e8b519e2a62ceab49228e", "Your x-axis is incorrect. Are you examining the sample plot correctly?"
    ...
    assert sha1(answer.encoding.y['shorthand'].encode('utf8')).hexdigest() == "30603fa9e0f620c305cd627ab0ff138a960c48bd", "Your x-axis is incorrect. Are you examining the plot sample correctly?"
...
def test_2_5(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert isinstance(answer, alt.vegalite.v5.api.VConcatChart), "Make sure you are concatenating both plots using the correct function."
    assert isinstance(answer.vconcat[0], alt.vegalite.v5.api.Chart), "Make sure the median plot is first."
    assert isinstance(answer.vconcat[1], alt.vegalite.v5.api.LayerChart), "Make sure the count plot is second."
    return(success)
...
def test_2_7_2(answer):
    ...
    assert answer.encoding['color']['shorthand'] == "when", "Make sure you are coloring using the 'when' column."
...
def test_3_4_1(answer):
    ...
    assert answer.encoding.x['shorthand'] == 'Year' or answer.encoding.x['field'] == 'Year', "Make sure you are plotting 'Year' on the x-axis."
    assert answer.encoding.y['shorthand'] == 'Firearm murders' or answer.encoding.y['field'] == 'Firearm murders', "Make sure you are plotting Firearm murders on the y-axis."
    assert answer.encoding['text']['value'].to_dict() == 'Gun law enacted', "make sure you are setting the value of the text appropriately."
...
def test_3_7_2(answer):
    ...
    assert answer.encoding['text']['value'].to_dict() == 'Gun law enacted', "make sure you are setting the value of the text appropriately for the text plot."
    return(success)
...
def test_4_4_3(answer):
    ...
    assert answer.encoding.y['shorthand'] == 'density:Q' or answer.encoding.y['field'] == 'density', "Make sure you are plotting 'density' on the y-axis."
    assert answer.encoding.y['stack'] == False, "Make sure unstack the y-axis so that the densities start from the same baseline."
    assert answer.encoding.x['axis']['format'] == "s", "Make sure you are formatting the x-axis to 's'."
    assert answer.encoding.color['shorthand'] == "where", "Make sure you are coloring using the 'where' column."
    return(success)
...
def test_4_4_4(answer):
    ...
    elif 'legend' in answer.to_dict()['encoding']['color']:
