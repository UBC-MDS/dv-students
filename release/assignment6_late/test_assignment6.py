...
def test_titles(answer):
    ...
    elif isinstance(answer['title'], alt.vegalite.v5.schema.core.TitleParams):
...
def test_1_5_1(answer):
    ...
    if 'text' in answer.to_dict()['encoding']:
        if 'field' in answer.to_dict()['encoding']['text']:
            assert answer.encoding['text']['shorthand'] == 'language', "Make sure you are setting the text to 'language' for the text plot."
        elif 'value' in answer.to_dict()['encoding']['text']: 
            assert answer.encoding['text']['value'].to_dict() == 'French', "Make sure you are setting the text value equal to 'French' for the text plot."
        else:
...
def test_1_5(answer):
    ...
    assert answer2.encoding.text['shorthand'] == 'language' or answer2.encoding.text['shorthand'] == 'language', "Make sure you are setting the text to 'language' for the text plot."
    if type(answer['title']) == str:
        assert answer['title'].count("_") == 0, "Make sure your plot title does not contain underscores."
        assert not answer['title'].islower(), "Make sure the plot title is capitalized."
    elif isinstance(answer['title'], alt.vegalite.v5.schema.core.TitleParams):
...
def test_3_5_2(answer):
    answer2_dict = answer.to_dict()['layer'][1]
    assert answer2_dict['encoding']['x']['field'] == 'information_rate' or answer2.encoding.x['field'] == 'information_rate', "Make sure you are plotting 'information_rate' on the x-axis for the text plot."
    assert answer2_dict['encoding']['x']['aggregate'] == 'mean', "Make sure you are taking the mean of the information_rate for the second plot."
    answer2 = answer.layer[1]
...
def test_4_3_1(answer):
    ...
    assert sha1(c2.to_dict()['encoding']['color']['field'].encode('utf8')).hexdigest() == '5430f4cf21194d437aee6e85dacdc28df35513f5', "Make sure you are coloring the map properly."
    return(success)

def test_4_3_2(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert answer.to_dict()['layer'][1]['encoding']['tooltip'][0]['field'] == 'language', "Make sure that you are filling 'language' in for the first tooltip field." 
    assert answer.to_dict()['layer'][1]['encoding']['tooltip'][1]['field'] == 'information_rate', "Make sure that you are filling 'information_rate' in for the second tooltip field."
    c2 = answer.layer[1]
...
def test_4_5_2(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    c2 = answer.layer[1]
    assert sha1(c2.encoding.latitude['shorthand'].encode('utf8')).hexdigest() == 'f1bd09c12f53c4adc7135f87451af543ec79a43d' or sha1(c2.encoding.latitude['shorthand'].encode('utf8')).hexdigest() == 'f1bd09c12f53c4adc7135f87451af543ec79a43d', "Make sure you are assigning the latitude correctly."
    assert sha1(c2.encoding.longitude['shorthand'].encode('utf8')).hexdigest() == '7c8c8b6917b098e739038a18ed3e75c4f92de9e1'or sha1(c2.encoding.longitude['shorthand'].encode('utf8')).hexdigest() == '7c8c8b6917b098e739038a18ed3e75c4f92de9e1', "Make sure you are assigning the longitude correctly."
    assert sha1(c2.encoding.size['shorthand'].encode('utf8')).hexdigest() == 'd047746b5ced84e37578c24bf7470a5ea1d27cb6' or sha1(c2.encoding.size['shorthand'].encode('utf8')).hexdigest() == 'd047746b5ced84e37578c24bf7470a5ea1d27cb6', "Make sure you are setting the size correctly."
    return(success)

def test_4_5_3(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    c1 = answer.to_dict()['layer'][0]
    c2 = answer.to_dict()['layer'][1]
    assert sha1(c2['encoding']['tooltip'][0]['field'].encode('utf8')).hexdigest() == 'e11523c5ff23fc1600aca2d8ee5adb542c5ce4b3', "Make sure you are setting the tooltip correctly."
    assert sha1(c2['encoding']['tooltip'][1]['field'].encode('utf8')).hexdigest() == 'd047746b5ced84e37578c24bf7470a5ea1d27cb6', "Make sure you are setting the tooltip correctly."
