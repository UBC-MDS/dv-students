...
def test_1_5a(answer):
    ...
    assert isinstance(answer, alt.Parameter), "Your answer for select_region should be an altair Selection object."
    assert answer.param != alt.utils.schemapi.Undefined and (
        answer.param.select.type == 'point'
    ), "Make sure to define select_region using 'selection_point()' function."
    assert answer.param.select.fields == ['region'], "Make sure you specify that you want to select on the 'region' fields. Hint: 'fields' argument should be a list."
    assert answer.param.bind == 'legend', "Make sure you specify that you want to bind the 'legend'."
    return("Success")
...
def test_1_5b(answer1, answer2):
    ...
    assert isinstance(answer1, alt.Parameter), "Your answer for select_region should be an altair Selection object."
    ...
    assert answer2.params != alt.utils.schemapi.Undefined and (
        answer1.name == answer2.params[0]['name'] and
        answer1.param.select == answer2.params[0].select
    ), "Make sure you add select_region to hw_legend_plot using 'add_params'."
    return("Success")
...
def test_1_5c(answer):
    ...
    assert (answer.encoding.opacity != alt.utils.schemapi.Undefined and 
            answer.params != alt.utils.schemapi.Undefined and
    ...
    assert answer['encoding']['opacity']['condition'].to_dict()['param'] in answer.params[0]['name'] and (
...
def test_2_2a(answer):
    ...
    assert type(answer) == alt.Parameter, "Your answer for interval should be an altair Selection object."
    assert answer.param.select != alt.utils.schemapi.Undefined and (
        answer.param.select.type == 'interval'
    ), "Make sure to define 'interval' using 'selection_interval()' function."
    assert "['x']" in str(answer.param.select), "Make sure to specify that you want to select on the 'x'-axis encoding."
    assert answer.param.select.encodings == ['x'], "Make sure you specify that you want to select on the 'x'-axis encoding. Hint: 'encodings' argument should be a list."
    return("Success")
...
def test_2_2b(answer1, answer2):
    ...
    assert type(answer1) == alt.Parameter, "Your answer for interval should be an altair Selection object."
    ...
    assert answer2.params != alt.utils.schemapi.Undefined and (
        answer1.name == answer2.params[0]['name'] and
        answer1.param.select == answer2.params[0].select
    ), "Make sure you add interval to bar_slider using 'add_params'."
    
    assert (answer2['encoding']['color'] != alt.utils.schemapi.Undefined and
            answer2.params != alt.utils.schemapi.Undefined and
    ...
    assert answer2['encoding']['color']['condition'].to_dict()['param'] == answer2.params[0].name and (
...
def test_2_2d(answer1, answer2, answer3):
    ...
    # Since altair 5, parameters are lifted out of individual plots
    # in combined views and added at top level instead
    if plot1.name != alt.utils.schemapi.Undefined:
        plot1.name = alt.utils.schemapi.Undefined
        plot1.params = answer3.params
        plot1.params[0]['views'] = alt.utils.schemapi.Undefined
    if plot2.name != alt.utils.schemapi.Undefined:
        plot2.name = alt.utils.schemapi.Undefined
        plot2.params = answer3.params
        plot2.params[0]['views'] = alt.utils.schemapi.Undefined
    assert (
        (answer1.to_dict()['params'][0]['select'] == answer3.to_dict()['params'][0]['select'])
        and (
            (plot1.to_dict() == answer1.to_dict() and plot2.to_dict() == answer2.to_dict())
            or
            (plot1.to_dict() == answer2.to_dict() and plot2.to_dict() == answer1.to_dict())
        )
    ), "Make sure you create dob_combo_plot by vertically stacking the bar_slider and scatter_plot."
    return("Success")
...
def test_3_3a(answer1, answer2):
    ...
    assert type(answer1) == alt.Parameter, "Your answer for state_select should be an altair Selection object."
    ...
    assert answer1.params != alt.utils.schemapi.Undefined and (
        answer1.params[0]['select']['type'] == 'multi'
    ), "Make sure to define state_select using 'selection_point()' function."
    assert answer1.params[0]['select']['fields'] == ['id'], "Make sure you specify that you want to select on the 'id' fields. Hint: 'fields' argument should be a list."
    ...
    assert (answer2.encoding.opacity != alt.utils.schemapi.Undefined and 
            ...
            answer2['encoding']['opacity']['condition']['param'] != alt.utils.schemapi.Undefined
           ), "Make sure you set the opacity channel to a condition depending on state_select."
    assert answer1.name == answer2['encoding']['opacity']['condition']['param'].to_dict() and (
...
def test3_3c(answer1, answer2, answer3):
    assert not answer1 is None, "Your answer for state_select_map does not exist. Have you passed in the correct variable?"
    assert type(answer1) == alt.LayerChart, "Your answer for state_select_map should be an altair LayerChart."
    assert not answer2 is None, "Your answer for pitching_scatter does not exist. Have you passed in the correct variable?"
    assert type(answer2) == alt.Chart, "Your answer for pitching_scatter should be an altair Chart."
    assert not answer3 is None, "Your answer for map_panel does not exist. Have you passed in the correct variable?"
    assert type(answer3) == alt.VConcatChart, "Your answer for map_panel should be an altair VConcatChart."

    assert type(answer3.vconcat) == list, "Your answer for map_panel should be an altair VConcatChart with 2 rows."
    assert len(answer3.vconcat) == 2, "Make sure your players_map LayerChart has 2 layers."

    assert answer1.params[0] == answer3.params[0], "The parameter of the individual states_select_map is not the same as in the combined map_panel"
    ...
    # Remove the param after testing that it is correct above
    plot3 = answer3.copy()
    # After confirming that the params are correct above, we can set them both to Undefined here so that the next comparisons work as they should (popping doesn't transfer that param correctly since it is top-level in combined charts and we can't transfer it manually since top-level param differ from chart level params).
    answer1 = answer1.copy()  # avoid removing the param from the actual chart that is used in the notebook
    answer1.params = alt.utils.schemapi.Undefined
    plot3.params = alt.utils.schemapi.Undefined
    # The two subplots have separate data from VConcatChart
    ...
    assert (
        (plot1.to_dict() == answer1.to_dict() and plot2.to_dict() == answer2.to_dict()) or
        (plot1.to_dict() == answer2.to_dict() and plot2.to_dict() == answer1.to_dict())
    ), "Make sure you create players_map by vertically stacking state_select_map and pitching_scatter."
    return("Success")
...
def test_4_1a(answer):
    ...
    assert (answer.mark != alt.utils.schemapi.Undefined and
            answer.mark.type == 'bar'), "Make sure you are using bar mark type."
    assert answer.mark.color != alt.utils.schemapi.Undefined, "Make sure you are giving the bar mark a color of your choice."
    
    assert answer.encoding.x != alt.utils.schemapi.Undefined and (
        answer.encoding.x['shorthand'] in {'count():quantitative', 'count():Q'} or (
            # Not a typo. Use 'count' instead of 'count()' when it is an aggregate instead of a field/shorthand
            answer.encoding.x['shorthand'] == 'count()' or answer.encoding.x['aggregate'] == 'count'
        )           
    ), "Make sure you use 'count()' as the X-axis encoding and specify that it is of 'quantitative' type."
    
    assert answer.encoding.y != alt.utils.schemapi.Undefined and (
        answer.encoding.y['shorthand'] in {'bats:nominal', 'bats:N'} or
        answer.encoding.y.type == 'nominal' and (
            answer.encoding.y['shorthand'] == 'bats' or answer.encoding.y['field'] == 'bats'
        )
    ), "Make sure you use 'bats' as the Y-axis encoding and specify that it is of 'nominal' type."
    return("Success")
...
def test_4_3b(answer1, answer2, answer3):
    ...
    assert answer1.params[0] == answer3.params[0], "The parameter of the individual states_select_map is not the same as in the combined map_bat_plot"
...
def test_5_3c(answer1, answer2):
    ...
    assert type(answer2) == alt.Parameter, "Your answer for select_team should be an altair Selection object."

    assert answer2.params != alt.utils.schemapi.Undefined and (
        answer2.param.select.type == 'point'
    ), "Make sure to define select_team using 'selection_point()' function."
    assert answer2.param.bind == answer1, "Make sure you bind dropdown_team to select_team."
    assert answer2.param.select.fields == ['team_name'], "Make sure you specify that you want to select on the 'team_name' fields. Hint: 'fields' argument should be a list."
    return("Success")
...
def test_5_3d(answer1, answer2):
    ...
    assert type(answer1) == alt.Parameter, "Your answer for select_team should be an altair Selection object."
    ...
    assert answer2.params != alt.utils.schemapi.Undefined and (
        answer1.name == answer2.params[0].name and
        answer1.param.select == answer2.params[0].select
    ), "Make sure you add select_team to stats_drop_plot using 'add_params'."
    ...
    assert answer2['encoding']['opacity']['condition'].to_dict()['param'] in answer2.params[0]['name'] and (
...
def test_5_5c(answer1, answer2, answer3):
    ...
    assert type(answer3) == alt.Parameter, "Your answer for select_bat_and_team should be an altair Selection object."

    assert answer3.params != alt.utils.schemapi.Undefined and (
        answer3.param.select.type == 'point'
    ), "Make sure to define select_bat_and_team using 'selection_point()' function."
    assert type(answer3.param.bind) == dict, "Make sure to set the bind argument for alt.selection_point() to a dictionary."
    assert 'team_name' in answer3.param.bind and (
        answer3['team_name'] == answer1
    ), "Make sure you bind dropdown_team to select_bat_and_team with 'team_name' as key."
    
    assert 'bats' in answer3.param.bind and (
        answer3['bats'] == answer1
    ), "Make sure you bind radio_batting to select_bat_and_team with 'bats' as key."

    assert type(answer3.param.select.fields) == list and (
        set(answer3.param.select.fields) == {"bats", "team_name"}
...
def test_5_5d(answer1, answer2):
    ...
    assert type(answer1) == alt.Parameter, "Your answer for select_bat_and_team should be an altair Selection object."
    ...
    assert answer2.params != alt.utils.schemapi.Undefined and (
        answer1.name == answer2.params[0].name and
        answer1.param.select == answer2.params[0].select
    ), "Make sure you add select_bat_and_team to stats_radio_plot using 'add_params'."
